...
import { cloudinary } from "@/shared/server/integrations/cloudinary"
...
export const getAvatarUploadSignature =
  async (): Promise<GetUploadSignatureResult> => {
    ...
    if (!apiKey || !cloudName || !apiSecret) {
      return { success: false, error: "Cloudinary server configuration error" }
    }
